...
The architecture preserves the following conceptual boundary:
...
This keeps the sample easy to deploy while still reflecting a four-agent pattern.
...
Agents do not access data tables directly. The evidence retrieval stage calls AgentCore Gateway tools. Gateway exposes only read-only operations:
...
Each tool call includes the active `exception_id`. The Gateway tool Lambda validates ID formats, rejects unexpected arguments, and refuses counterparty, account, root-cause, or playbook requests that do not match the active exception case.
...
The sample emits enough telemetry to reconstruct a case run:
